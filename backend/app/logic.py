...
def refresh_knowledge_base() -> None:
    """Reload the BM25 retriever from the default docs and uploaded files."""
...
def _compose_session_title_from_text(text: str) -> str:
    ...
    stopwords = {
        ...
        "what", "how", "can", "you", "are", "our", "will", "have", "been",
        "does", "doesnt", "give", "given", "new", "document", "pdf", "file", "help",
    }
    keywords = []
    seen = set()
    for token in tokens:
...
def _extract_contact_details(messages: List[BaseMessage]) -> tuple[str, str]:
    ...
    if not name and email:
        disallowed_tokens = {
            ...
            "okay", "ok", "email", "name", "my", "is", "am", "im",
        }
...
def _retrieve(query: str, max_results: int = 6) -> List[Document]:
    if not knowledge_base_docs or bm25_retriever is None:
        return []
...
def router_node(state: GraphState):
    ...
    is_short_greeting = (
        _is_greeting(latest_message)
        and len(normalized_latest.split()) <= 2
        and not any(char.isdigit() for char in normalized_latest)
    )
...
def lead_qualify_node(state: GraphState):
    """Persist a qualified lead when both name and email are present."""
...
def greet_user_node(state: GraphState):
    ...
    return {
        "messages": [
            AIMessage(content="Hi! How can I help you learn more about Shahzaib?")
        ]
    }
...
def fallback_response_node(state: GraphState):
    ...
    return {
        "messages": [
            AIMessage(content="I can answer questions about Shahzaib's skills, experience, and projects. What would you like to know?")
        ]
    }


def initialize_rag() -> None:
    """Initializes the BM25 retriever from the default docs plus uploads."""
...
def retrieve_docs(state: GraphState):
    ...
    documents = _retrieve(user_query)
    context = "\n\n".join(document.page_content for document in documents)
    return {"context": context, "retrieved_docs": documents}


def generate_answer(state: GraphState):
    """Answer with the retrieved context and conversation history."""
